...
export function EdgeConfigDisplay({ data }: EdgeConfigDisplayProps) {
    ...
    return (
      <div className="rounded-lg border border-dashed p-6 text-center">
        <p className="text-muted-foreground">
          No Edge Config data found. Make sure EDGE_CONFIG is set and the store
          contains the expected keys.
        </p>
        <p className="mt-2 text-muted-foreground text-sm">
          Expected keys: <code>regional_banner_by_country</code>,{" "}
          <code>blocked_skus</code>, <code>redirect_rules</code>,{" "}
          <code>sale_page_version</code>
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        {data.regional_banner_by_country ? (
          <div className="space-y-2">
            {Object.entries(data.regional_banner_by_country).map(
              ([country, banner]) => (
                <div
      ...
      {/* Blocked SKUs */}
      <div>
        <h4 className="mb-2 font-semibold text-sm">blocked_skus</h4>
        {data.blocked_skus?.length ? (
      ...
      {/* Redirect Rules */}
      <div>
        <h4 className="mb-2 font-semibold text-sm">redirect_rules</h4>
        {data.redirect_rules?.length ? (
          <div className="space-y-1 font-mono text-sm">
            {data.redirect_rules.map((rule) => (
              <div
      ...
      {/* Sale Page Version */}
      <div>
        <h4 className="mb-2 font-semibold text-sm">sale_page_version</h4>
        {data.sale_page_version ? (
          <Badge>{data.sale_page_version}</Badge>
        ) : (
          <p className="text-muted-foreground text-sm italic">
            Not set (defaults to v1)
          </p>
        )}
      </div>
    </div>
